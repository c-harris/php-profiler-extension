--TEST--
Algoweb: Test excluding call_user_func and similar functions
--SKIPIF--
<?php
...
$algoweb_ignored_functions = array( 'ignored_functions' => 
...
algoweb_enable(0, $algoweb_ignored_functions);
foo("this is a test");
$array_arg = array();
$array_arg[] = 'calling ';
$array_arg[] = 'foo_array';
...
algoweb_enable(ALGOWEB_FLAGS_CPU, $algoweb_ignored_functions);
...
$tmp1 = $algoweb_ignored_functions['ignored_functions'];
$tmp1[] = 'strlen';
$ignore_strlen_also = array('ignored_functions' => $tmp1);
algoweb_enable(ALGOWEB_FLAGS_MEMORY, $ignore_strlen_also);
...
algoweb_enable(ALGOWEB_FLAGS_MEMORY, array());
test_my_call_user_func_array_safe('foo_array');
$output = algoweb_disable();
echo "Part 5b output:\n";
print_canonical($output);
...
$algoweb_ignored_functions = array('ignored_functions' => 
                                  'my_call_user_func_array_safe');
algoweb_enable(ALGOWEB_FLAGS_MEMORY, $algoweb_ignored_functions);
test_my_call_user_func_array_safe('foo_array');
$output = algoweb_disable();
echo "Part 5c output:\n";
print_canonical($output);
